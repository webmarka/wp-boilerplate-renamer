	...
	class WPPB_Renamer {
			...
			foreach ( $items as $item ) {
				...
				if ( in_array( $item, $excluded_files ) ) {
					...
					*/
					...
					if ( $renamefiles['modified'] )
						$this->renamed_files_count++;
		...
		public function display_results( $shell = false, $verbose = false ) {
			...
			foreach ( $this->results as $path => $infos ) {
				...
				// Check conditions.
				if ( isset( $infos['excluded'] ) && $infos['excluded'] === true ) {
					$color = $infos['modified'] ? 'blue' : 'white';
				} elseif ( isset( $infos['is_dir'] ) && $infos['is_dir'] === true ) {
					$color = isset( $infos['modified'] ) && $infos['modified'] ? 'blue' : 'white';
					$before = $shell ? " " : "";
				} else {
					$color = isset( $infos['modified'] ) && $infos['modified'] ? 'blue' : 'white';
					$before = $shell ? "\t" : "";
				...
				$final_item = isset( $infos['modified'] ) && $infos['modified'] ? $infos['new_name'] : $infos['item'];

				// Prepare line output.
				if ( $shell ) {
					if ( $verbose && !empty( $infos['item'] ) )
						$output .= $before . $this->shell_text( $infos['item'], $color ) . " " . $infos['message'] . "\n";
					elseif ( $infos['excluded'] !== true && !empty( $infos['item'] ) )
						$output .= $before . $this->shell_text( $final_item, $color ) . "\n";
				} else {
					$final_item = isset( $infos['is_dir'] ) && $infos['is_dir'] ? "<b>" . $final_item . "</b>" : $final_item;
					if ( $verbose && !empty( $infos['item'] ) )
						$output .= "<li style=\"color:{$color};\">" . $infos['item'] . " " . $infos['message'] . "</li>";
					elseif ( $infos['excluded'] !== true && !empty( $infos['item'] ) )
						$output .= "<li style=\"color:{$color};\">" . $final_item . "</li>";
				}
			}
